...
it will be necessary to use the `tmp` folder on `/data/horse`. In order to ensure that the files in
the temporary folder will be owned by root, it is necessary to set up an image inside
`/data/horse/tmp` instead of using it directly. E.g., to create a 25 GB of temporary memory image:
