...
the architecture (e.g. `x86`). Therefore, the packages should be installed using interactive
...
!!! note
    ...
    Inside a Jupyterlab session you must specify the library installation path.
    Please use workspaces.

```bash
export RLIB_DIR=/data/horse/ws/<marie>-rpacks
R -e "install.packages(c('ggplot2'), repos='http://cran.r-project.org', lib="$RLIB_DIR")"
```
...
[`Capella`](../jobs_and_resources/hardware_overview.md#capella) and
[`Alpha`](../jobs_and_resources/hardware_overview.md#alpha-centauri) is beneficial for the examples
here.
