...
- Core dumps are useful when your program crashes after a long
  runtime with a segmentation fault (segfault).
    - The creation of core dumps must be explicitly enabled with the command `ulimit -c <size>`.
    - Core files are created in the current directory with the name `.core.<hostname>.<pid>`.
