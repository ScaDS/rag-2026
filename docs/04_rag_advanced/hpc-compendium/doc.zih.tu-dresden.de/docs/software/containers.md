...
In some cases using Singularity may require a Linux machine with root privileges,
the same architecture and a compatible kernel.
For many reasons, users on ZIH systems cannot be granted root permissions.
To work with Virtual Machines on ZIH systems, please follow the Instructions on [Virtual Machines](virtual_machines.md)
